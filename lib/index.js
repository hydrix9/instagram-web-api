...
class Instagram {
  ...
  async login({ username, password } = {}, { _sharedData = true } = {}) {
    ...
    // Provide _sharedData
    if (_sharedData) {
      this._sharedData = await this._getSharedData(username)
    }

    return res.body
  }

  async _getSharedData(username) {
    return this.request("/users/web_profile_info/?username=" + username)
      .then(html => { return JSON.parse(JSON.stringify(html.data.user)) })
  }

  async _getGis(path, username) {
    const { rhx_gis } = this._sharedData || (await this._getSharedData(username))
  ...
  async getUserByUsername({ username }) {
    return this.request({
      ...
      headers: {
        ...
        'x-instagram-gis': await this._getGis(`/${username}/`, username)
      }
